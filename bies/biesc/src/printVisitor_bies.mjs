...
class BytecodeVisitor extends BiesVisitor {
    ...
    visitValueExpression(ctx) {
        ...
        const determineOperation = (leftType, rightType, operation) => {
            const operationMap = {
                '+': { 'INT': 'ADD ; Sumar entero', 'FLOAT': 'ADD ; Sumar decimal', 'STRING': 'CAT ; Concatenar cadenas', 'INDEX': 'ADD ; Sumar entero'},
                '-': { 'INT': 'SUB ; Restar', 'FLOAT': 'SUB ; Restar', 'INDEX': 'SUB ; Restar' },
                '*': { 'INT': 'MUL ; Multiplicar', 'FLOAT': 'MUL ; Multiplicar', 'INDEX': 'MUL ; Multiplicar' },
                '/': { 'INT': 'DIV ; Dividir', 'FLOAT': 'DIV ; Dividir', 'INDEX': 'DIV ; Dividir' },
        ...
        const processExpression = (expr) => {
            ...
            if (expr.children && expr.children.length > 2 && expr.children[1].getText() === '[') {
            ...
            if (expr.children && expr.children.length > 1 && expr.children[3] != ']') {
