...
class BytecodeVisitor extends BiesVisitor {
  ...
  visitFunctionCall(ctx) {
    ...
    if (
      ...
      (functionData?.type !== 'function' &&
        functionData?.type !== 'nestedLambda')
    ) {
  ...
  evaluateRPN(rpn, ctx) {
    ...
    const findNode = (value, listName) => {
      return this.findNodeByText(ctx, value)
        ...
        : ctx.children[
              this.findIndexInList(
                this.variableMap.get(listName)
                  ? this.variableMap.get(listName)[1].getText()
                  : '',
                value,
              )
            ]
    ...
    const operationFunctions = {
      '+': (a, b, x, y) => {
        if (x || y) {
          ...
          if (x?.getText() !== flag || y?.getText() !== flag)
    ...
    if (rpn.length == 1) {
      ...
      const x = this.findNodeByText(ctx, a)
        ? this.findNodeByText(ctx, a)
        : null;
      if (x) {
        this.loadValue(x);
      } else {
        throw new Error(
          'Error inesperado intentando retornar el valor de una lista',
        );
      }
    }
  ...
  compareResults(steps, x1, y1) {
    ...
    return (
      (x1 == lastResult && y1 == secondLastResult) ||
      (x1 == secondLastResult && y1 == lastResult)
    );
  }
